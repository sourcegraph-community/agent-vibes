...
import { generateSummariesCommandHandler, type GenerateSummariesCommandInput } from './index';
...
export const generateSummariesEndpoint = async (request: Request): Promise<Response> => {
  ...
  let payload: Partial<GenerateSummariesCommandInput> = {};

  if (isJsonRequest(request)) {
    try {
      payload = (await request.json()) as Partial<GenerateSummariesCommandInput>;
    } catch (error) {
